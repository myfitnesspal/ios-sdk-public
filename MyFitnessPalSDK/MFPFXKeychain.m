...
@implementation NSObject (MFPFXKeychainPropertyListCoding)

- (id)MFPFXKeychain_propertyListRepresentation
{
    return self;
}
...
#if !MFPFXKeychain_USE_NSCODING

@implementation NSNull (MFPFXKeychainPropertyListCoding)
...
- (id)MFPFXKeychain_propertyListRepresentation
{
    ...
    for (id obj in self)
    {
        id value = [obj MFPFXKeychain_propertyListRepresentation];
...
+ (instancetype)defaultKeychain
{
    ...
    dispatch_once(&onceToken, ^{
        ...
        NSString *bundleID = [[NSBundle bundleForClass:[self class]] objectForInfoDictionaryKey:(NSString *)kCFBundleIdentifierKey];
        sharedInstance = [[MFPFXKeychain alloc] initWithService:bundleID
                                                 accessGroup:nil];
    });
          ...
          accessGroup:(NSString *)accessGroup
{
    return [self initWithService:service
                   ...
                   accessibility:MFPFXKeychainAccessibleWhenUnlocked];
}

- (id)initWithService:(NSString *)service
          accessGroup:(NSString *)accessGroup
        accessibility:(MFPFXKeychainAccess)accessibility
{
    if ((self = [super init]))
    {
...
- (NSData *)dataForKey:(id)key
{
    ...
    {
		NSLog(@"MFPFXKeychain failed to retrieve data for key '%@', error: %ld", key, (long)status);
	}
...
- (BOOL)setObject:(id)object forKey:(id)key
{
    ...
    if (object && !data)
    {
        data = [NSPropertyListSerialization dataWithPropertyList:[object MFPFXKeychain_propertyListRepresentation]
    ...
    NSAssert(!object || (object && data), @"MFPFXKeychain failed to encode object for key '%@', error: %@", key, error);
    ...
    if (data)
    {
        ...
        {
            NSLog(@"MFPFXKeychain failed to store data for key '%@', error: %ld", key, (long)status);
            return NO;
        }
    }
    else if (self[key])
    {
        ...
        if (status != errSecSuccess)
        {
            NSLog(@"MFPFXKeychain failed to delete data for key '%@', error: %ld", key, (long)status);
            return NO;
        }
    }
    return YES;
...
- (id)objectForKey:(id)key
{
    ...
    if (data)
    {
        ...
        if (!object)
        {
             NSLog(@"MFPFXKeychain failed to decode data for key '%@', error: %@", key, error);
        }
        return object;
    }
    else
